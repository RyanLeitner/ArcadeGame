...
{   private ArrayList<Choppers> gunners;
	...
	public BackGroundScene()
	{
		...
		gunners = new ArrayList<Choppers>();
		keys = new boolean[5];
	...
	for (int i = 0;i<gcount;i++){
		...
		gunners.add(new Choppers(xpos,ypos,50,30,10));
		xpos+=150;
   ...
   public void update(Graphics window)
   {
	...
	public void paint( Graphics window )
	{
		...
		for (int n =0;n<gunners.size();n++){
			gunners.get(n).draw(backgroundImage);
		}
		...
		for (int j=0;j<bullets.size();j++){
			for (int a =0;a<gunners.size();a++){
				if (gunners.get(a).didHit(bullets.get(j)) == true)
				{gunners.get(a).setHeight(0);
				gunners.get(a).setWidth(0);
		...
		//add in collision detection to see if Bullets hit the gunners and if Bullets hit the Ship
